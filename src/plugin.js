...
const addAuditLogObject = (currentObject, original, action, options = {}) => {
  ...
  const changes = getDiffData(original, currentObject, filter);
  const auditLog = buildAuditLog(currentObject, original, changes, user, action);
...
const buildAuditLog = (currentObject, original, changes, user, action) => {
  return {
    ...
    originalDocument: currentObject,
    user
  };
};
...
const addInsertMany = (currentObject, options, action, next) => {
  const orig = Object.assign({}, currentObject._doc || currentObject);
  orig.constructor.modelName = currentObject.constructor.modelName;
...
const addFindAndDelete = (query, action, next, options = {}) => {
  query.find()
    .lean(true)
    .cursor()
    .eachAsync(fromDb => addDelete(fromDb, Object.assign({}, query.options, options), action, next))
    .then(() => next())
    .catch(next);
};
...
const plugin = function Audit(schema, options = {}) {
  ...
  schema.pre('deleteMany', function (next) {
    addFindAndDelete(this, 'deleteMany', next,{hasStorage});
  });

  schema.pre('insert', function (next) {
    addInsert(this, {hasStorage},'insert', next);
  });

  schema.pre('insertMany', function (next, docs) {
    addInsertMany(docs, {hasStorage} ,'insert' ,next)
  });
};
